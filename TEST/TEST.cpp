...
volatile bool step=0;                //zmienna krok używana do znalezienia odpowiedniej instrukcji ustawienia serwo. 

//tablice, w których zapisane są kolejności kroków dla poszczególnych rodzajów ruchu
const uint8_t forward []  = {0,1,3};
...
//    pos[0]=90;
//    pos[2]=90;
  if(step)//case w zależności od kroku w podanej tablicy
    {
...
{ 
  ...
  if (pos[0]>=0&&pos[0]<=180)//pierwsze servo
    {
       OCR1A = pos[0]*(range[1]-range[0])/180+range[0];
    }

  if (pos[1]>=0&&pos[1]<=180)//drugie servo
    {
       OCR1B = pos[1]*(range[1]-range[0])/180 + range[0];
    }
  if (pos[2]>=0&&pos[2]<=180)//trzecie servo
    {
       OCR2A = pos[2]*(range[1]-range[0])/180+range[0];
    }
}
